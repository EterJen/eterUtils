...
public class SolarTermsUtil {
    ...
    public SolarTermsUtil(LocalDateTime localDateTime) {
        gregorianYear = localDateTime.getYear();
        gregorianMonth = localDateTime.getMonthValue();
        gregorianDate = localDateTime.getDayOfMonth();
    ...
    public static String getSolarQuarterStr(Date date) {
        int solarQuarter = getSolarQuarter(date);
        String res = "计算出现错误";
        switch (solarQuarter) {
            case 1:
                res = "春季";
    ...
    public static void main(String[] args) {
        String firstDay = "2021-02-12 00:00:00";

        DateTimeFormatter dateFmt = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime today = LocalDateTime.parse(firstDay, dateFmt);
        //userViewSkinInsert.setSkin();
        ...
        System.out.println(solarTerms + ":" + solarTermsCode + ":" + today);

        for (String s : solarTermsUtil.getAdjacentThreeSolarTermCodes(solarTermsCode)) {
            System.out.println(s);
        }
        //当前日期所在 阴历季度
        System.out.println(getSolarQuarter(DateTimeUtils.DATE_TIME.parse("2117-08-06 00:00:01")));

        //System.out.println(getSolarTermsOfMonth(2034, 2));
    }
}